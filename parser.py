...
def generic_chinese_intrans_verb(index):
    return {
        ...
        "PRE_RULES": [
            ...
            FiberRule(DISINHIBIT, VERB, SUBJ, 0),
            FiberRule(DISINHIBIT, VERB, ADVERB, 0),
            FiberRule(DISINHIBIT, VERB, ADJ, 0),
            AreaRule(DISINHIBIT, ADVERB, 1),
        ],
        "POST_RULES": [
            FiberRule(INHIBIT, LEX, VERB, 0),
...
CHINESE_LEXEME_DICT = {
    ...
    # Treat as stative verb for "你真善良"
    "善良": generic_chinese_intrans_verb(12),
    "无可奈何地": generic_chinese_adverb(13),
    "愤怒地": generic_chinese_adverb(14),
    "真": generic_chinese_adverb(15),
    "一颗": generic_chinese_quantifier(16),
    "温柔": generic_chinese_adjective(17),
    "大度": generic_chinese_adjective(18),
}
...
class ParserDebugger():
    ...
    def peak(self):
            ...
            for _, to_area_list in test_proj_map.items():
                for to_area in to_area_list:
                    ...
                    if not self.b.area_by_name[to_area].saved_winners:
                        self.b.area_by_name[to_area].saved_winners.append(
                            self.b.area_by_name[to_area].winners)
        ...
        for area, num_test_projects in remove_map.items():
            ...
            self.b.area_by_name[area].saved_w = self.b.area_by_name[area].saved_w[:(
                -num_test_projects)]
...
def parseHelper(b, sentence, p, LEX_k, project_rounds, verbose, debug,
    ...
    allowed_words = set(sentence)
    token_pos = {w: i for i, w in enumerate(sentence)}

    def decode_top_words_from_lex_winners(top_n=5, min_overlap=0.2, allowed=None):
        if not b.area_by_name[LEX].winners:
            return []
        winners = set(b.area_by_name[LEX].winners)
        area_k = b.area_by_name[LEX].k
        threshold = int(min_overlap * area_k)
        scored = []
        ...
        scored.sort(key=lambda t: (-t[0], token_pos.get(t[1], 10**9)))
        return [w for _, w in scored[:top_n]]

    def decode_single_word_from_lex_winners(min_overlap=0.2, allowed=None):
        words = decode_top_words_from_lex_winners(
            top_n=1, min_overlap=min_overlap, allowed=allowed)
    ...
    if readout_method == ReadoutMethod.FIBER_READOUT:
        ...
        # Chinese: improve readout for consecutive predicate adjectives like "你真温柔善良大度"
        # Expected: adverb modifies all adjectives; subject is the subject of all adjectives.
        if isinstance(b, ChineseParserBrain) and (OBJ not in activated_fibers.get(VERB, set())):
            # Read subject from SUBJ
            subj = None
            b.project({}, {VERB: [SUBJ]})
            b.project({}, {SUBJ: [LEX]})
            subj = decode_single_word_from_lex_winners(
                min_overlap=0.2, allowed=allowed_words)

            # Read predicate head from VERB (avoid mis-decoding the subject as the head)
            b.project({}, {VERB: [LEX]})
            head_candidates = decode_top_words_from_lex_winners(
                top_n=3, min_overlap=0.2, allowed=allowed_words)
            head = None
            ...
            # Read adverbs
            adverbs = []
            b.project({}, {VERB: [ADVERB]})
            b.project({}, {ADVERB: [LEX]})
            adverbs = decode_top_words_from_lex_winners(
                top_n=3, min_overlap=0.2, allowed=allowed_words)

            # Read adjectives (may be a mixed assembly; decode multiple candidates restricted to sentence words)
            adjectives = []
            b.project({}, {VERB: [ADJ]})
            b.project({}, {ADJ: [LEX]})
            adjectives = decode_top_words_from_lex_winners(
                top_n=10, min_overlap=0.15, allowed=allowed_words)

            # Treat the predicate head as one of the predicate adjectives
            all_pred_adjs = []
            if head:
                all_pred_adjs.append(head)
            all_pred_adjs.extend(
                [w for w in adjectives if w not in all_pred_adjs])
            all_pred_adjs.sort(key=lambda w: token_pos.get(w, 10**9))

            # Avoid emitting predicate dependencies for the subject/adverbs if they get decoded as ADJ noise.
            exclude_words = set([subj])
            exclude_words.update(adverbs)
            all_pred_adjs = [
                w for w in all_pred_adjs if w not in exclude_words]

            for adj in all_pred_adjs:
                if subj:
                    dependencies.append([adj, subj, SUBJ])
                for adv in adverbs:
                    dependencies.append([adj, adv, ADVERB])
